...
Let the bolt group have $n$ bolts at coordinates $(x_i, y_i)$ measured from the centroid, and let:
...
$$
\sum r^2 = \sum_{i=1}^{n} (x_i^2 + y_i^2)
$$
...
* ($\sum r^2$): Polar “second moment” of the bolt pattern (mm²·mm²)

The direction of $F_{m,i}$ is tangential to the circle about the centroid (perpendicular to the radius). Its components are:

$$
F_{mx,i} = -F_{m,i} \frac{y_i}{r_i}
$$
$$
F_{my,i} = F_{m,i} \frac{x_i}{r_i}
$$

* ($F_{mx,i}$): x-component of force in bolt $i$ due to moment (N)
...
* Shear components:
  $$
  V_{ix} = V_{p,x} + F_{mx,i}
  $$
  $$
  V_{iy} = V_{p,y} + F_{my,i}
  $$

  where $V_{p,x}$ and $V_{p,y}$ are the components of the uniform shear per bolt.
