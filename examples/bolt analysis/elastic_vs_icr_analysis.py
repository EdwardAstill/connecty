...
def summarize(result_name: str, result, load_label: str) -> str:
    ...
    if result.critical_index is not None and result.critical_bolt is not None:
        ...
        lines.append(
            f"  Critical bolt #{result.critical_index + 1} at (y={bf.y:.1f}, z={bf.z:.1f}) -> R={bf.resultant:.2f} kN"
        )
    lines.append("  Per-bolt forces (kN):")
    for i, bf in enumerate(result.bolt_forces, start=1):
        lines.append(
            f"    {i:>2}: Fy={bf.Fy:.2f}, Fz={bf.Fz:.2f}, R={bf.resultant:.2f}, angle={bf.angle:.1f} deg"
        )
    return "\n".join(lines)
...
def run() -> None:
    ...
    load_rect = Load(Fy=-120000, location=(0, 90, 140))  # 120 kN down, eccentric in z
    ...
    load_circ = Load(Fy=-90000, Fz=45000, location=(0, 0, 80))
    ...
    report_lines = [
        ...
        "=" * 60,
        summarize("Rectangular – Elastic", rect_elastic, "Fy=120 kN @ z=140 mm"),
        "",
        summarize("Rectangular – ICR", rect_icr, "Fy=120 kN @ z=140 mm"),
        "",
        summarize("Circular – Elastic", circ_elastic, "Fy=90 kN + Fz=45 kN @ z=80 mm"),
        "",
        summarize("Circular – ICR", circ_icr, "Fy=90 kN + Fz=45 kN @ z=80 mm"),
    ]
