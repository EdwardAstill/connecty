...
from common.common_geometry import (
    get_rhs_weld,
    get_ibeam_weld,
    get_channel_weld,
    get_chs_weld,
)
from common.common_loads import get_vertical_shear, get_combined_load
...
def run():
    ...
    loaded.plot(section=True, force=True, save_path=str(gallery_dir / "rhs_simple_vertical_load.svg"), show=False)
    ...
    loaded = LoadedWeld(weld, load, method="elastic")
    print(f"   Max stress: {loaded.max:.1f} MPa")
    loaded.plot(section=True, force=True, save_path=str(gallery_dir / "i_beam_combined_load.svg"), show=False)
    ...
    loaded = LoadedWeld(weld, load, method="elastic")
    print(f"   Max stress: {loaded.max:.1f} MPa")
    loaded.plot(section=True, force=True, save_path=str(gallery_dir / "u_channel_vertical_load.svg"), show=False)
    ...
    loaded = LoadedWeld(weld, load, method="elastic")
    print(f"   Max stress: {loaded.max:.1f} MPa")
    loaded.plot(section=True, force=True, save_path=str(gallery_dir / "chs_vertical_load.svg"), show=False)
